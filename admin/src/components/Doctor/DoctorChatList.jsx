...
const DoctorChatList = ({ onChatSelect, selectedAppointmentId }) => {
    const { userChats, fetchUserChats } = useContext(DoctorChatContext);
    ...
    return (
        <div className="bg-white rounded-lg shadow-md border h-full flex flex-col">
            <div className="p-4 border-b"><h2 className="text-xl font-semibold text-gray-800">Patient Messages</h2></div>
            <div className="overflow-y-auto">
                {userChats.length > 0 ? userChats.map(chat => (
                    ...
                    >
                        <img src={chat.appointmentId.userData.image} alt={chat.appointmentId.userData.name} className="w-14 h-14 rounded-full object-cover"/>
                        <div className="flex-1 overflow-hidden">
                            <div className="flex justify-between items-center">
                                <h3 className="font-semibold text-gray-900 truncate">{chat.appointmentId.userData.name}</h3>
                                <p className="text-xs text-gray-500">{new Date(chat.lastMessageTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                            </div>
